...
class PayoutsResponseRaw(Transact,models.Model):
...
class RawPayouts(Transact,models.Model):
	...
	result_status = models.IntegerField(default=Transact.STATUS_INITIAL)
	result_description = models.TextField(max_length=100, blank=True, null=True)
	notes = models.TextField(blank=True, null=True)
	transaction_id = models.CharField(max_length=50,null=True)
	created_at = models.DateTimeField(default=timezone.now)

	@classmethod
	def create_payout(cls,msisdn,amount,reference_number,key):
		merchant=Merchant.objects.get(pk=1)
		return cls.objects.create(msisdn=msisdn,amount=int(amount),reference_number=reference_number,merchant=merchant,key=key)
	@classmethod
	def reference_number_exists(cls,reference_number):
		return cls.objects.filter(reference_number=reference_number).exists()
	...
	def get_unprocessed_results(cls,limit):
		return cls.objects.filter(result_status=Transact.STATUS_INITIAL).filter(results__isnull=False).order_by('id')[:limit]

	@classmethod
	def update_payout(cls,result_code,result_description,originator_converstation_id,transaction_id,detail):
		return cls.objects.filter(originator_converstation_id=originator_converstation_id)\
		.update(result_code=result_code,result_description=result_description,transaction_id=transaction_id,results=detail)
...
class PayoutsSourceLog(models.Model):
